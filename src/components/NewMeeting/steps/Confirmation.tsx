...
const Confirmation: React.FC<ConfirmationProps> = ({ onPrev, formData }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Make API call
      const response = await api.post("/appointments/api/admin/appointments/", formDataToSend, {
        headers: {
      ...
      setErrorMessage(error.response?.data?.message || "Failed to create meeting. Please try again.");
      setShowError(true);
    } finally {
